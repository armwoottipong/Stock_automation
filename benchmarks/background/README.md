...
`isolated_generation.json` freezes the seven white-background fixture prompts. `isolated_cases.json` lists those local files and drives the primary BiRefNet/BEN2 comparison. `cases.json` is the earlier mixed-scene robustness set. Their inputs and model outputs live in ignored `input/` and `output/`; the measured decision is in `docs/phase-5-results.md`.
