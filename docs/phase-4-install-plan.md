# Phase 4 upscale install plan — 2026-09-25
...
The two model files require user approval under the supplied setup document's model download flow. On this machine, drive D had about 94.8 GB free at planning time. The downloaded model files and node checkout stay outside Git. Verify file size and SHA-256 after download, pin the node revision, and register source, license and installed state. The publisher page gives SHA-256 `9f23ba7be22bf8796c12565e00ea4b287acac982cdf384d368a8b18b6990e011` for the ControlNet file. The Real-ESRGAN release has no published digest in its listing, so record a local digest and verify the release asset size.
