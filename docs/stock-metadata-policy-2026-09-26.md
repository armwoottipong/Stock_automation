...
The later 4× fruit set has its own `metadata_x4/` catalog, Adobe CSV and `embedded_draft/` PNG copies. These describe the 4× transparent cutouts and retain the same generative-AI provenance gate. The 4× opaque images in `upscaled_x4/` are working images without buyer-facing embedded metadata; their white-background titles would need a separate catalog and Adobe photo-format preparation if selected for submission.
