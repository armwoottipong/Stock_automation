...
The [stock metadata policy check](docs/stock-metadata-policy-2026-09-26.md) documents Adobe Stock and Shutterstock CSV fields and eligibility. `scripts/export_stock_metadata.py` validates an English catalog and matching asset names before export. `scripts/embed_stock_metadata.py` writes XMP title, description and keywords to review copies and verifies pixel identity. Both block Shutterstock output for generative-AI catalogs. The 25 generated fruit cutouts have a local Adobe CSV draft and embedded XMP PNG copies in `staging/fruit_isolates_2026-09-26/metadata/`; they still require visual review and Adobe's AI disclosure in the Contributor Portal.

## Current pixel upscale default

`python controller.py upscale --input path\to\image.png` now enlarges 4× with the registered RealESRGAN x4plus model. The [five-image fruit comparison](docs/x4-upscale-evaluation-2026-09-26.md) supports this provisional choice for isolated fruit. Use `--scale 2 --model-id realesrgan-x2plus` for 2×. Structured router jobs also freeze 4× by default; existing frozen jobs keep their saved scale and model. The 25-fruit 4× files are in `staging/fruit_isolates_2026-09-26/upscaled_x4/` and `cutout_x4/` after processing.

The local `staging/packages/fruit_isolates_x4_2026-09-26.zip` contains the 25 white-background working PNGs, 25 transparent PNGs with embedded XMP, Adobe CSV draft and a delivery manifest. It excludes duplicate untagged cutouts. The ZIP is a review package, not a cleared stock submission. The [artifact lifecycle](docs/artifact-lifecycle.md) keeps `output/` for approved packages only; `python scripts/audit_output.py` checks it. Model and system comparisons are indexed in the [benchmark dashboard](benchmarks/README.md).
