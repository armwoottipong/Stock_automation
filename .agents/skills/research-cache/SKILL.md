...
For an expired, missing or invalid entry, review official model and license sources and the local benchmark before preparing an update. Read all six `data/*registry.json` files and `data/research_cache.json`. Update relevant registry verification dates and prepare a reviewed `ResearchEntry` JSON with a validity window of at most 30 days. Run `python scripts/research_cache.py upsert --entry <file>` only after the evidence is recorded. Do not infer commercial eligibility from the cache alone, and never research within a running render job.
